...
const formatOrders = (orders) => {
  ...
  return orders
    .map(o => {
      ...
      <td><b>Ordered at ${dp4(o.orderPrice)} ${quoteCurrency}</b></td>
...
const expectedProfit = (o) => {
  const priceRatio = o.side == 'sell' ? o.fillPrice/o.priceAtCreation : o.priceAtCreation/o.fillPrice
...
assert('actualProfit 0', 0, actualProfit({ side:'sell', priceAtCreation:1000, fillPrice:1000, fees:0 }))
assert('actualProfit 100% on sell no fees', 100, actualProfit({ side:'sell', priceAtCreation:1000, fillPrice:2000, fees:0 }))
assert('actualProfit 10% on sell no fees', 10, actualProfit({ side:'sell', priceAtCreation:1000, fillPrice:1100, fees:0 }))
assert('actualProfit 50% on sell with fees', 50, actualProfit({ side:'sell', priceAtCreation:1000, fillPrice:2000, fees:500 }))
assert('actualProfit 90% on sell with fees', 90, actualProfit({ side:'sell', priceAtCreation:1000, fillPrice:2000, fees:100 }))
assert('actualProfit 100% on buy no fees', 100, actualProfit({ side:'buy', priceAtCreation:2000, fillPrice:1000, fees:0 }))
assert('actualProfit 10% on buy no fees', 10, actualProfit({ side:'buy', priceAtCreation:1100, fillPrice:1000, fees:0 }))
assert('actualProfit 90% on buy with fees', 90, actualProfit({ side:'buy', priceAtCreation:2000, fillPrice:1000, fees:100 }))
